...
import { Loader2, Mail, Lock, LogIn, Sparkles, ArrowLeft, AlertCircle, Eye, EyeOff, Sun, Moon } from "lucide-react";
...
export default function Login() {
  const { login, googleLogin, addToast, theme, toggleTheme } = useAuth();
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      addToast("Logged in via Google.", "success");
      navigate("/");
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="text-3xl font-black tracking-tight text-indigo-950 dark:text-white leading-none">Welcome Back</h1>
          <p className="text-xs text-indigo-900/60 dark:text-slate-400 font-bold tracking-tight">Log in to initialize your high-tech whiteboard workspace.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link to="/forgot-password" onClick={() => playTechSound(700, "triangle", 0.08, 0.015)} className="text-[10px] font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition">
                Forgot Password?
              </Link>
            </div>
            <div className="relative">
          ...
          <div className="flex items-center gap-2.5 px-1 py-1">
            <input
              ...
              id="remember-me"
              checked={rememberMe}
              onChange={(e) => { setRememberMe(e.target.checked); playTechSound(e.target.checked ? 720 : 420, "triangle", 0.06, 0.01); }}
              className="h-4 w-4 rounded border-indigo-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-indigo-500 accent-indigo-500 cursor-pointer"
            />
            <label htmlFor="remember-me" className="text-[11px] font-bold text-indigo-900/60 dark:text-slate-400 uppercase tracking-wide cursor-pointer select-none">
              Remember node session (7 days)
            </label>
          </div>
          ...
          >
            {loading ? <Loader2 size={16} className="animate-spin" /> : <><LogIn size={16} /><span>Log In</span></>}
          </motion.button>
        </form>
        ...
        <div className="w-full flex justify-center [&>div]:w-full [&_iframe]:w-full overflow-hidden rounded-2xl">
          <GoogleLogin
            ...
            onError={() => setError("Google login failed. Please ensure you have a valid Client ID.")}
        ...
        <p className="text-center text-xs text-indigo-900/60 dark:text-slate-400 font-semibold tracking-tight">
          ...
          <Link to="/signup" onClick={() => playTechSound(700, "triangle", 0.08, 0.015)} className="font-black text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 underline underline-offset-4">
            Register Workspace
          </Link>
        </p>
      </motion.div>
